  ...
  const getservicedata= ()=>
  {
    axios.get("https://devapi.saltworld.co/api/services/",).then((response)=>{
      ...
      for (let services in response.data){
        service.set(response.data[services].ServiceName,response.data[services])
  ...
  useEffect(()=>{
    ...
    },
    {
      ...
      const handleSubmit=async (e)=>{
        ...
        axios.post('http://localhost:6622/api/memberships', recievedData, {
    ...
    <div className="  justify-center">
      { errormessage.length>0?
        ...
        <div className=' grid justify-items-stretch grid-cols-2 gap-4'>
          ...
          <div className=''><label class="block text-gray-700 text-sm font-bold mb-2">Membership Description</label>
          <textarea name="MembershipDescription" placeholder="Membership Description" className="shadow appearance-none border border-grey-500 rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"/>
          </div>
          ...
          <input  name='isunlimited' value={isun} onChange={()=>{setisun(!isun)}} type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"></input>
          <label for="checked-checkbox" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-900">Is Unlimited</label>
          </div>
          </div>
          ...
          <thead>
            ...
            <th class="font-bold py-2 px-4 border-b border-l border-r  text-left">Count</th>
            </tr>
          </thead>
          <tbody>
          ...
          {/* <tr>
            ...
            <button className='w-[400px] my-5 py-2 bg-teal-600  text-white font-semibold rounded-lg' type="submit">SUBMIT</button>
        </form>
